...
export function SpellingPracticeCard({
  allWords,
  userId,
}: SpellingPracticeCardProps) {
  const [practiceWords, setPracticeWords] = useState<
    (Word & Partial<WordProgress>)[]
  >([]);
  const [unusedWords, setUnusedWords] = useState<
    (Word & Partial<WordProgress>)[]
  >([]);
  const [currentWord, setCurrentWord] = useState<
    (Word & Partial<WordProgress>) | null
  >(null);
  ...
  useEffect(() => {
    ...
    setUnusedWords(words); // تعبئة الطابور بالبداية
  }, [allWords, userId]);

  const uniqueUnits = useMemo(() => {
    return Array.from(
      new Set(practiceWords.map((word) => word.unit).filter(Boolean))
    );
  }, [practiceWords]);
  ...
  const selectNewWord = useCallback(() => {
    ...
    if (pool.length === 0) {
      pool = [...filteredPracticeWords]; // إعادة ملء الطابور
    }
    ...
    const [nextWord, ...rest] = pool; // أخذ أول كلمة
    setCurrentWord(nextWord);
    setAnswer("");
    setFeedback("idle");
    setUnusedWords(rest); // تحديث الطابور
  }, [filteredPracticeWords, spelledToday, unusedWords]);

  // ✅ استدعاء آمن بدون loop
  useEffect(() => {
    if (filteredPracticeWords.length > 0 && !currentWord) {
      setUnusedWords([...filteredPracticeWords]);
      const [first, ...rest] = filteredPracticeWords;
      setCurrentWord(first);
      setUnusedWords(rest);
    }
  }, [filteredPracticeWords, currentWord]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (correct) {
      ...
      setTimeout(() => {
        ...
        // 👇 إرجاع الكلمة الخاطئة إلى نهاية الطابور
        setUnusedWords((prev) => [
          ...prev,
          currentWord as Word & Partial<WordProgress>,
        ]);
        ...
        <div className="flex items-center space-x-2 pt-4">
          ...
          {(selectedUnit || selectedLesson) && (
            <Button variant="ghost" onClick={clearFilters}>
              {t("spellingPractice.clearFilters")}
            </Button>
          )}
        </div>
      </CardHeader>
        ...
        {spelledToday >= DAILY_LIMIT ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="flex items-center gap-2">
              ...
              <Button type="submit" disabled={feedback !== "idle"}>
                {t("spellingPractice.check")}
              </Button>
            </form>
          </>
        ) : (
          <div className="text-center text-muted-foreground py-8">
            <p>
              {filteredPracticeWords.length > 0
                ? t("spellingPractice.loading")
                : t("spellingPractice.noWordsForFilters")}
            </p>
          </div>
        )}
      </CardContent>
