...
export default function TodaysReviewsPage() {
  ...
  const loadNextWord = useCallback(async () => {
    if (userId) {
      const nextWord = await getWordForReview(userId, null, null, "today");
      setWord(nextWord);
      if (!nextWord) {
  ...
  const handleUpdateStats = useCallback(
    async (reviewedCount: number, durationSeconds: number) => {
      if (userId) {
  ...
  const handleCorrect = async (option: ScheduleOption) => {
    ...
    await updateStudentProgressInStorage(userId, word.id, {
          ...
          {sessionFinished ? (
            <div className="text-center p-8 bg-card rounded-lg shadow-lg">
              <h2 className="text-2xl font-bold font-headline mb-4">
                {t("learn.sessionFinished.title")}
              </h2>
              <p className="text-muted-foreground mb-6">
                {t("learn.sessionFinished.description")}
              </p>
              <Button asChild>
                <Link href={`/dashboard?userId=${userId}`}>
